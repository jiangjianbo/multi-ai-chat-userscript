...
- 主窗口：包含多个内容块的新页面，该窗口名字为`multi-ai-chat-main-window`
...
研究的目的是将技术难题进行分解，形成稳定的局部技术方案。在真正形成代码和测试用例的时候，可以直接使用研究的成果。

例如：需要形成一个复杂的界面。如果将界面和代码逻辑放在一起的话，会造成极大的干扰，很难一次性成功。因此可以将复杂界面分解成一个或多个研究，每个研究负责调整提示词，生成界面的一部分。而且这个生成的结果是可以被开发者查看和调整的。在研究成功结束之后，这个复杂界面的代码就能够直接使用在源代码中，而且还可以在单元测试中被引用或参考。

`Webpack`有专门的`research`任务，用于处理研究代码并形成最终研究的成果。

详细的研究工作在[research.md](./research.md)文档中说明。

## 整体架构

### 模块划分

以下是模块的名称、模块英文名（也是文件名）和模块的功能描述。
...
* 主窗口控制器（main-window-controller）：负责多AI模型同时对话和结果对比的核心逻辑。
* 主窗口创建器（sync-chat-window）：负责主窗口的创建、检查和样式注入。
