...
本模块比较重要，需要进行一些基础技术的研究。本系列研究主要是在测试用例成功的基础上，将代码封装成油猴的脚本，使得开发人员可以直观感知和在浏览器中调试功能的完整性。

这些功能需要使用`webpack`将功能打包成油猴脚本，存放在`dist`目录下。

### 按钮注入

最终脚本名称为`button.userscript.js`。

脚本根据页面自动选择合适的`PageDriver`，并在页面注入按钮“同步对比”，点击按钮弹出对话框显示“Hello”。要注意因为不同的大模型对话页面布局不同，按钮要避开页面已有元素，避免重叠。

### 工具条注入

最终脚本名称为`toolbar.userscript.js`。

在“按钮注入”成功的基础上，点击按钮生成工具条和每个回答区域的工具条，并且可以正常工作。

### 主窗口创建与通讯

最终脚本名称为`create-window.userscript.js`。

在“按钮注入”成功的基础上，点击按钮则检测主窗口是否生成，如果生成则激活主窗口，否则就生成主窗口，并在主窗口中显示“Hello”

### 原生窗口的内容附加

最终脚本名称为`element.userscript.js`。

在“按钮注入”成功的基础上，点击按钮将PageDriver支持的所有元素设置为红色边框。
