...
主要目的是为了验证“同步按钮”的样式和注入逻辑。脚本名称为`button`。

脚本根据页面自动选择合适的`PageDriver`，并在页面注入按钮“同步对比”，点击按钮弹出对话框显示“Hello”。要注意因为不同的大模型对话页面布局不同，按钮要避开页面已有元素，避免重叠。

### 问答工具条注入

主要目的是为了验证问答工具条的样式和注入方法的正确性。脚本名称为`toolbar`。

在“按钮注入”成功的基础上，点击按钮生成每个问题和答案区的工具条，并且可以正常工作。

### 索引工具条注入

主要目的是为了验证问答内容的索引工具条的样式和注入方法的正确性。脚本名称为`qa-index`。

在“按钮注入”成功的基础上，点击按钮为每个答案生成一个索引并支持跳转。
